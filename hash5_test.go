...
func TestH5Hash(t *testing.T) {
	...
	for _, s := range inputs {
		v := h.hash([]byte(s), 0)
		if v >= h5BucketSize {
			t.Errorf("h5.hash(%q) = %d, exceeds h5BucketSize %d", s, v, h5BucketSize)
		}
	}
	...
	if a == b {
		t.Errorf("h5.hash collision: %q and %q both hash to %d", "ABCD", "EFGH", a)
	}
}

func TestH5StoreAndRetrieve(t *testing.T) {
...
func TestH5FindLongestMatch(t *testing.T) {
...
func TestH5FindLongestMatchCacheHit(t *testing.T) {
...
func TestH5NoMatchFallsToDict(t *testing.T) {
...
func TestH5StitchToPreviousBlock(t *testing.T) {
	const bufSize = 128
	data := make([]byte, bufSize)
...
func TestPrepareDistanceCache(t *testing.T) {
	distCache := make([]int, 16)
	distCache[0] = 10
	distCache[1] = 20
	distCache[2] = 30
	distCache[3] = 40
	...
	// Check derived entries from distCache[0]=10.
	want := []int{9, 11, 8, 12, 7, 13}
	for i, w := range want {
		if distCache[4+i] != w {
			t.Errorf("distCache[%d] = %d, want %d", 4+i, distCache[4+i], w)
		}
	}
}

func TestBackwardReferencePenalty(t *testing.T) {
	// Penalty for cache[0] reuse is not applied (i==0 case in findLongestMatch).
	// For cache[1] (i=1), penalty should be > 0.
	p1 := backwardReferencePenaltyUsingLastDistance(1)
	if p1 == 0 {
		t.Error("expected non-zero penalty for distance short code 1")
	}

	// Penalty for i=0 doesn't matter (never called), but verify the function
	// doesn't panic.
	p0 := backwardReferencePenaltyUsingLastDistance(0)
	_ = p0
}
