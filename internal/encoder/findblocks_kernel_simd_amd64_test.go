...
func TestFindBlocksStepMatchesTheTwoKernelsItReplaces(t *testing.T) {
	for _, shape := range findBlocksKernelShapes {
		for _, n := range findBlocksKernelSizes {
			costA, insertA := findBlocksKernelFixture(t, n, shape)
			costB := append([]float64(nil), costA...)
			insertB := append([]float64(nil), insertA...)
			bitmapLen := (n + 7) >> 3
			sigA := make([]byte, bitmapLen)
			sigB := make([]byte, bitmapLen)

			minA, bestA := findBlocksStep(costA, insertA, sigA, findBlocksTestSwitchCost)
			minB, bestB := findBlocksDPStep(costB, insertB)
			findBlocksClamp(costB, sigB, minB, findBlocksTestSwitchCost)

			if math.Float64bits(minA) != math.Float64bits(minB) || bestA != bestB {
				t.Fatalf("shape=%s n=%d: fused returned (%#016x, %d), two kernels (%#016x, %d); "+
					"best is stored as the block type, so a difference changes the output",
					shape, n, math.Float64bits(minA), bestA, math.Float64bits(minB), bestB)
			}
			for i := range costB {
				if math.Float64bits(costA[i]) != math.Float64bits(costB[i]) {
					t.Fatalf("shape=%s n=%d index %d: fused cost bits %#016x, two kernels %#016x",
						shape, n, i, math.Float64bits(costA[i]), math.Float64bits(costB[i]))
				}
			}
			for i := range sigB {
				if sigA[i] != sigB[i] {
					t.Fatalf("shape=%s n=%d sig[%d]=%#02x, two kernels %#02x; the switch bitmap "+
						"drives the backtrace", shape, n, i, sigA[i], sigB[i])
				}
			}
		}
	}
...
func benchmarkFindBlocksStep(b *testing.B, n int) {
	cost, insertCost := findBlocksKernelFixture(b, n, "mixed")
	sig := make([]byte, (n+7)>>3)
	b.Run("impl=before_two_kernels", func(b *testing.B) {
		b.ReportAllocs()
		for range b.N {
			findBlocksMinSink, findBlocksBestSink = findBlocksDPStep(cost, insertCost)
			findBlocksClamp(cost, sig, findBlocksMinSink, findBlocksTestSwitchCost)
		}
	})
	b.Run("impl=after_fused", func(b *testing.B) {
		b.ReportAllocs()
		for range b.N {
			findBlocksMinSink, findBlocksBestSink = findBlocksStep(cost, insertCost, sig,
				findBlocksTestSwitchCost)
		}
	})
}

func BenchmarkFindBlocksStep100Histograms(b *testing.B) { benchmarkFindBlocksStep(b, 100) }
func BenchmarkFindBlocksStep50Histograms(b *testing.B)  { benchmarkFindBlocksStep(b, 50) }
