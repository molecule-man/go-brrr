// H5 hasher variant with bucketBits=15, blockBits=6 for quality 7.
//
// Compared to H5b5 (quality 6), this variant doubles the bucket count
// (32K vs 16K) and quadruples the per-bucket depth (64 vs 32 entries),
// giving the encoder a much deeper match search. The distance cache is
// also expanded: 10 entries are checked instead of 4, including derived
// near-miss distances for the two most recent backward references.
...
const (
	h5b6BucketBits = 15
	h5b6BucketSize = 1 << h5b6BucketBits // 32768
	h5b6BlockBits  = 6
	h5b6BlockSize  = 1 << h5b6BlockBits // 64
	h5b6BlockMask  = h5b6BlockSize - 1
	h5b6HashShift  = 32 - h5b6BucketBits // 17

	// h5b6HashTypeLength is the minimum number of bytes needed to compute
	// the hash and verify a match (StoreLookahead in C).
	h5b6HashTypeLength = 4
)

// h5b6 is the H5 hasher with bucketBits=15 and blockBits=6: a forgetful hash
// table where each of 32K buckets holds a ring buffer of up to 64 positions.
type h5b6 struct {
	num        [h5b6BucketSize]uint16                 // entry count per bucket
	buckets    [h5b6BucketSize * h5b6BlockSize]uint32 // position ring buffers
...
func (h *h5b6) reset(oneShot bool, inputSize uint, data []byte) {
...
// data[pos & mask].
func (h *h5b6) store(data []byte, mask, pos uint) {
...
func (h *h5b6) storeRange(data []byte, mask, start, end uint) {
	for i := start; i < end; i++ {
		h.store(data, mask, i)
	}
}

func (h *h5b6) storeNoWrap(data []byte, pos uint) {
	key := h.hash(data, pos)
	minorIx := h.num[key] & h5b6BlockMask
	offset := uint(minorIx) + uint(key)<<h5b6BlockBits
	h.num[key]++
...
func (h *h5b6) storeRangeNoWrap(data []byte, start, end uint) {
	for i := start; i < end; i++ {
...
func (h *h5b6) stitchToPreviousBlock(numBytes, position uint, ringBuffer []byte, ringBufferMask uint) {
...
func (h *h5b6) findLongestMatchSmallBuf(
	data []byte, ringBufferMask uint,
	distCache *[16]uint,
	cur, maxLength, maxBackward, dictDistance uint,
	dictNumLookups, dictNumMatches *uint,
...
func (h *h5b6) createBackwardReferences(s *encodeState, bytes, wrappedPos uint32) {
	mask := uint(s.mask)
...
func (h *h5b6) createBackwardReferencesNoWrap(s *encodeState, bytes, wrappedPos uint32) {
	data := s.data
	mask := uint(s.mask)
	maxBackwardLimit := (uint(1) << s.lgwin) - core.WindowGap
	gap := s.compound.totalSize
...
func (h *h5b6) findLongestMatchNoWrap(
	data []byte,
	distCache *[16]uint,
	cur, maxLength, maxBackward, dictDistance uint,
	dictNumLookups, dictNumMatches *uint,
