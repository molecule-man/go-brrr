// H6 hasher variant with blockBits=6 for quality 7, large inputs with large windows.
//
// Compared to H6b5 (quality 6), this variant doubles the per-bucket depth
// (64 vs 32 entries) and expands the distance cache from 4 to 10 entries,
// giving the encoder deeper match search and better distance reuse.
//
// Selected when quality=7, sizeHint >= 1MiB, and lgwin >= 19.
...
const (
	h6b6BucketBits = 15
	h6b6BucketSize = 1 << h6b6BucketBits // 32768
	h6b6BlockBits  = 6
	h6b6BlockSize  = 1 << h6b6BlockBits // 64
	h6b6BlockMask  = h6b6BlockSize - 1
	h6b6HashShift  = 64 - h6b6BucketBits // 49

	// h6b6HashTypeLength is the minimum number of bytes needed to compute
	// the hash and verify a match (StoreLookahead in C).
	h6b6HashTypeLength = 8

	// h6b6NumLastDistances is the number of distance cache entries to check.
	// For quality 7–8, the C reference uses 10.
	h6b6NumLastDistances = 10
)

// h6b6HashMul is the hash multiplier: kHashMul64 << (64 - 5*8).
// Pre-computed because the untyped shift overflows Go constant arithmetic.
const h6b6HashMul uint64 = 0x7BD3579BD3000000

// h6b6 is the H6 hasher with blockBits=6: a forgetful hash table where each
// of 32K buckets holds a ring buffer of up to 64 positions.
type h6b6 struct {
	num        [h6b6BucketSize]uint16                 // entry count per bucket
	buckets    [h6b6BucketSize * h6b6BlockSize]uint32 // position ring buffers
...
func (h *h6b6) hash(data []byte, i uint) uint32 {
	return uint32((loadU64LE(data, i) * h6b6HashMul) >> h6b6HashShift)
}
...
func (h *h6b6) reset(oneShot bool, inputSize uint, data []byte) {
	partialPrepareThreshold := h6b6BucketSize >> 6
...
// data[pos & mask].
func (h *h6b6) store(data []byte, mask, pos uint) {
	key := h.hash(data, pos&mask)
	minorIx := h.num[key] & h6b6BlockMask
	offset := uint(minorIx) + uint(key)<<h6b6BlockBits
	h.num[key]++
	h.buckets[offset] = uint32(pos)
}

// storeRange records positions [start, end) in the hash table.
func (h *h6b6) storeRange(data []byte, mask, start, end uint) {
	for i := start; i < end; i++ {
		h.store(data, mask, i)
	}
}
...
func (h *h6b6) stitchToPreviousBlock(numBytes, position uint, ringBuffer []byte, ringBufferMask uint) {
	if numBytes >= h6b6HashTypeLength-1 && position >= 3 {
...
//  2. Hash bucket scan: walk the ring buffer of up to 64 positions for the
...
func (h *h6b6) findLongestMatchSmallBuf(
	data []byte, ringBufferMask uint,
	distCache *[16]int,
	cur, maxLength, maxBackward, dictDistance uint,
	dictNumLookups, dictNumMatches *uint,
...
func (h *h6b6) createBackwardReferences(s *encodeState, bytes, wrappedPos uint32) {
	data := s.data
	mask := uint(s.mask)
	...
	storeEnd := position
	if uint(bytes) >= h6b6HashTypeLength {
		storeEnd = posEnd - h6b6HashTypeLength + 1
	}
		...
		if sr.score > minScore {
			...
			for {
				...
				if sr2.score >= sr.score+costDiffLazy {
					...
					if delayedBackwardReferencesInRow < 4 &&
						position+h6b6HashTypeLength < posEnd {
						maxLength--
						continue
					}
				}
			...
			if position > applyRandomHeuristics {
				if position > applyRandomHeuristics+4*randomHeuristicsWindowSize {
					posJump := min(position+16, posEnd-max(h6b6HashTypeLength-1, 4))
					...
					posJump := min(position+8, posEnd-(h6b6HashTypeLength-1))
					for position < posJump {
						h.store(data, mask, position)
						insertLength += 2
						position += 2
